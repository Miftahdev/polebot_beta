...
def calculation():
    ...
    while not rospy.is_shutdown():
        ...
        Error_X = round((koordinat_X - posisi_X),2)#/7
        Error_Y = round((koordinat_Y - posisi_Y), 2)#/7
        omega = round((posisi_T),1)
        ...
        MVX = min(max(MVX,-0.04),0.04) # batas jarak
        MVY = min(max(MVY,-0.02),0.02)
        ...
        # V1 = min(max(V1,-0.02),0.02) # batas pada setiap motor
        # V2 = min(max(V2,-0.04),0.04)
        # V3 = min(max(V3,-0.02),0.02)

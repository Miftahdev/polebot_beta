...
void set_point_callback(const std_msgs::Float64& set_point_1) {
  set_point = set_point_1.data;
}

void pulse_callback(const std_msgs::Int64& pulse_1) {
  int pulse_pub = 0;
  pulse_pub = pulse_1.data;
}

ros::Subscriber<std_msgs::Float64> sub("/V1", &set_point_callback);
ros::Publisher encoderValue("/encoderValue_1", &encoder);

void setup() {
  nh.getHardware()->setBaud(57600);//baudrate
  nh.initNode();
  nh.subscribe(sub);
...
//    new_Vel = Vel;/
    ...
    error = abs(set_point) - abs(new_RPS);
    integral += error * dt;

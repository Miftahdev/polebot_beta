...
def calculation():
    ...
    while not rospy.is_shutdown():
        ...
        Error_X = round((koordinat_X - posisi_X),3)#/7
        Error_Y = round((koordinat_Y - posisi_Y),3)#/7
        ...
        pub_reset_enc = rospy.Publisher('/encoder_reset', Int64, queue_size = 10)

        pub_V1.publish(V1)
        pub_V2.publish(V2)
        pub_V3.publish(V3)

        if Error_X or Error_Y > 0 :
            pub_reset_enc.publish(0)
